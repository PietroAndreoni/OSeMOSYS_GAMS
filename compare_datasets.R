...
ggplot(Production %>% 
         filter( (str_detect(scen,select_multiple_scens)) & data %in% data_select & FUEL %in% primary & storage==want_storage) %>%
...
ggplot(TotalCost %>% 
         filter( (str_detect(scen,select_multiple_scens)|scen=="base") & data %in% data_select  & storage==want_storage) %>%
         group_by(YEAR,scen) %>%
